...
type Props = { currentQuestion:Question; currentQuestionIndex:number; selectedOption:string|null; isOptionsDisabled:boolean; score:number; totalQuestions:number; onOptionPress:(option:string)=>void; onNextQuestion:()=>void; onGoHome:()=>void };
const LETTERS = ['A','B','C','D'];

export default function QuizScreen({ currentQuestion, currentQuestionIndex, selectedOption, isOptionsDisabled, score, totalQuestions, onOptionPress, onNextQuestion, onGoHome }: Props) {
  const insets=useSafeAreaInsets(); const {width}=useWindowDimensions(); const wide=width>=700;
  const item=currentQuestion; const isAnswered=isOptionsDisabled; const progress=((currentQuestionIndex+1)/totalQuestions)*100;
